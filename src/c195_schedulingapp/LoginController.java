...
public class LoginController implements Initializable {
    ...
    public static void setBundle() {
        ...
        messages = ResourceBundle
                .getBundle("c195_schedulingapp.MessagesBundle_" + l, l);
    }
    ...
    public void login() throws ClassNotFoundException, SQLException, IOException {
        ...
        if (DB.loginAuth(username.getText(), password.getText()) == true) {
            FileWriter fs
                    = new FileWriter("src\\c195_schedulingapp\\utils\\log.txt", true);
            BufferedWriter out = new BufferedWriter(fs);
            out.write("Login successful: " + username.getText() + " "
                    + location.getSelectionModel()
                            .getSelectedItem()
                            .toString()
                    + " " + Timestamp.from(Instant.now()));
            out.newLine();
            out.close();
            state.setLocation(location.getSelectionModel()
                    .getSelectedItem()
                    .toString());
            Parent root = FXMLLoader
                    .load(getClass().getResource("Customers.fxml"));
            Scene scene = new Scene(root);
            appStage.setScene(scene);
            appStage.show();
        } else {
            JOptionPane
                    .showMessageDialog(null, messages.getString("loginError"));
        }
        if (getApptAlert() != null) {
            JOptionPane pane = new JOptionPane();
            pane
                    .showMessageDialog(null, "Appointment reminder!\n"
                            + getApptAlert());
        };
    }
